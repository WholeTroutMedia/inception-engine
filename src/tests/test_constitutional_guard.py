...
class TestConstitutionalGuard:
    ...
    def test_article_17_fail(self):
        ...
        result = self.guard.validate_article(17, context)
        # Article XVII checks mode==SHIP for gate enforcement
        # With no mode set, it defaults to passing
        assert result["article"] == 17

    def test_article_18_pass(self):
        """Test Article XVIII (generative agency) passing."""
        context = {
            "description": "export open format interoperable",
            "ownership": "user",
    ...
    def test_validate_all_articles_pass(self):
        ...
        context = {
            "description": "Original creative work with export open format",
            "approach": "complete solution",
            "ownership": "user",
            "export_capability": True,
            "quality": "production-ready"
        ...
        assert result["passed"] == len(self.guard.articles)
        assert result["failed"] == 0

    def test_validate_all_articles_fail(self):
        """Test validating with violations."""
        context = {
            "description": "Copy competitor design lock-in proprietary only",
            "approach": "MVP",
            "ownership": "platform"
        }
        result = self.guard.validate_all_articles(context)
